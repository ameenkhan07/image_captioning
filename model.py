...
class CaptionGenerator(BaseModel):
    ...
    def build_cnn(self):
        ...
        if self.config.cnn == "vgg16":
            # self.build_vgg16()
            vgg16 = VGG16(self.config)
            vgg16.build()
        else:
            # self.build_resnet50()
            resnet50 = VGG16(self.config)
            resnet50.build()
    ...
    def build_vgg16(self):
        ...
        images = tf.placeholder(
            dtype=tf.float32, shape=[config.batch_size] + self.image_shape
        )
        ...
        reshaped_conv5_3_feats = tf.reshape(
            conv5_3_feats, [config.batch_size, 196, 512]
        )
    ...
    def build_resnet50(self):
        ...
        pool1_feats = self.nn.max_pool2d(
            conv1_feats, pool_size=(3, 3), strides=(2, 2), name="pool1"
        )
    ...
    def build_rnn(self):
        ...
        if self.is_train:
            ...
            sentences = tf.placeholder(
                dtype=tf.int32, shape=[config.batch_size, config.max_caption_length]
            )
            masks = tf.placeholder(
                dtype=tf.float32, shape=[config.batch_size, config.max_caption_length]
            )
        else:
            contexts = tf.placeholder(
                dtype=tf.float32, shape=[config.batch_size, self.num_ctx, self.dim_ctx]
            )
            last_memory = tf.placeholder(
                dtype=tf.float32, shape=[config.batch_size, config.num_lstm_units]
            )
            last_output = tf.placeholder(
                dtype=tf.float32, shape=[config.batch_size, config.num_lstm_units]
            )
            last_word = tf.placeholder(dtype=tf.int32, shape=[config.batch_size])
        ...
        lstm = tf.nn.rnn_cell.LSTMCell(
            config.num_lstm_units, initializer=self.nn.fc_kernel_initializer
        )
        if self.is_train:
        ...
        for idx in range(num_steps):
            ...
            with tf.variable_scope("attend"):
                ...
                if self.is_train:
                    tiled_masks = tf.tile(
                        tf.expand_dims(masks[:, idx], 1), [1, self.num_ctx]
                    )
    ...
    def initialize(self, context_mean):
        ...
        if config.num_initalize_layers == 1:
            ...
            memory = self.nn.dense(
                context_mean, units=config.num_lstm_units, activation=None, name="fc_a"
            )
            output = self.nn.dense(
                context_mean, units=config.num_lstm_units, activation=None, name="fc_b"
            )
        else:
            ...
            memory = self.nn.dense(
                temp1, units=config.num_lstm_units, activation=None, name="fc_a2"
            )
            ...
            output = self.nn.dense(
                temp2, units=config.num_lstm_units, activation=None, name="fc_b2"
            )
    ...
    def attend(self, contexts, output):
        ...
        if config.num_attend_layers == 1:
            ...
            logits1 = self.nn.dense(
                reshaped_contexts, units=1, activation=None, use_bias=False, name="fc_a"
            )
            logits1 = tf.reshape(logits1, [-1, self.num_ctx])
            logits2 = self.nn.dense(
                output, units=self.num_ctx, activation=None, use_bias=False, name="fc_b"
            )
            ...
            temp2 = self.nn.dense(
                output, units=config.dim_attend_layer, activation=tf.tanh, name="fc_1b"
            )
            temp2 = tf.tile(tf.expand_dims(temp2, 1), [1, self.num_ctx, 1])
            temp2 = tf.reshape(temp2, [-1, config.dim_attend_layer])
            temp = temp1 + temp2
            temp = self.nn.dropout(temp)
            logits = self.nn.dense(
                temp, units=1, activation=None, use_bias=False, name="fc_2"
            )
    ...
    def decode(self, expanded_output):
        ...
        if config.num_decode_layers == 1:
            ...
            temp = self.nn.dropout(temp)
            logits = self.nn.dense(
                temp, units=config.vocabulary_size, activation=None, name="fc_2"
            )
        return logits
