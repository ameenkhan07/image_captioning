...
class DataSet(object):
    ...
    def next_batch(self):
        ...
        if self.has_full_next_batch():
            ...
            current_idxs = self.idxs[start:end] + list(
                np.random.choice(self.count, self.fake_count)
            )
...
def prepare_train_data(config):
    ...
    if not os.path.exists(config.temp_annotation_file):
        ...
        image_files = [
            os.path.join(config.train_image_dir, coco.imgs[image_id]["file_name"])
    ...
    dataset = DataSet(
        image_ids, image_files, config.batch_size, word_idxs, masks, True, True
    )
    print("Dataset built.")
    return dataset
...
def prepare_test_data(config):
    ...
    image_files = [
        os.path.join(config.test_image_dir, f)
        for f in files
