...
class NN(object):
    ...
    def max_pool2d(self, inputs, pool_size=(2, 2), strides=(2, 2), name=None):
    ...
    def dense(self, inputs, units, activation=tf.tanh, use_bias=True, name=None):
    ...
    def dropout(self, inputs, name=None):
        """ Dropout layer. """
        return tf.layers.dropout(
            inputs=inputs, rate=self.config.fc_drop_rate, training=self.is_train
        )

    def batch_norm(self, inputs, name=None):
        """ Batch normalization layer. """
        return tf.layers.batch_normalization(
            inputs=inputs, training=self.train_cnn, trainable=self.train_cnn, name=name
        )
