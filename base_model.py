        ...
        for _ in tqdm(list(range(config.num_epochs)), desc="epoch"):
            for _ in tqdm(list(range(train_data.num_batches)), desc="batch"):
                ...
                _, summary, global_step = sess.run(
                    [self.opt_op, self.summary, self.global_step], feed_dict=feed_dict
                )
    ...
    def eval(self, sess, eval_gt_coco, eval_data, vocabulary):
        ...
        for k in tqdm(list(range(eval_data.num_batches)), desc="batch"):
            ...
            for l in range(eval_data.batch_size - fake_cnt):
                ...
                results.append(
                    {"image_id": eval_data.image_ids[idx].item(), "caption": caption}
                )
                idx += 1
                ...
                if config.save_eval_result_as_image:
                    ...
                    plt.title(caption)
                    plt.savefig(
                        os.path.join(config.eval_result_dir, image_name + "_result.jpg")
                    )
    ...
    def test(self, sess, test_data, vocabulary):
        ...
        for k in tqdm(list(range(test_data.num_batches)), desc="path"):
            ...
            for l in range(test_data.batch_size - fake_cnt):
                ...
                plt.title(caption)
                plt.savefig(
                    os.path.join(config.test_result_dir, image_name + "_result.jpg")
                )

        # Save the captions to a file
        results = pd.DataFrame(
            {"image_files": test_data.image_files, "caption": captions, "prob": scores}
        )
        ...
        for idx in range(config.max_caption_length):
            ...
            for b in range(num_steps):
                if idx == 0:
                    ...
                    last_word = np.array(
                        [pcl[b].sentence[-1] for pcl in partial_caption_data_lists],
                        np.int32,
                    )

                last_memory = np.array(
                    [pcl[b].memory for pcl in partial_caption_data_lists], np.float32
                )
                last_output = np.array(
                    [pcl[b].output for pcl in partial_caption_data_lists], np.float32
                )
